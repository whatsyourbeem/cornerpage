...
1. **입력 정보 파악** — 사용자가 준 사업 정보를 확인한다. 부족하면 `references/input-questions.md`의 문항 순서대로 물어본다. 이미 대화에 정보가 있으면 다시 묻지 않는다.
...
2. **업종 2축 분류 판단** — 아래 "업종 분류"로 톤(axis_a)과 레이아웃(axis_b)을 정한다. 이게 이후 모든 판단의 뿌리다.
3. **블록 온·오프 및 순서 결정** — 데이터가 있는 블록만 켠다. 데이터 없는 선택 블록은 JSON에서 `null`. 축 B에 따라 갤러리/메뉴 순서를 정한다.
4. **콘텐츠 생성** — 각 블록의 카피를 "카피 생성 원칙"에 따라 작성한다. 근거 없는 최상급 표현은 항상 사실 기반으로 다듬는다.
5. **JSON 출력** — `references/schema.md`의 스키마에 정확히 맞춰 출력한다. 필수 필드 누락 금지, 없는 데이터는 `null`.
...
5. **강점 체크리스트가 빈약하면 카피도 빈약해진다.** 강점이 1~2개뿐이면 `references/industry-data.md`의 업종별 기본 강점 후보를 참고해 각도를 넓힌다. 그래도 사실이 아닌 건 넣지 않는다.
...
- **`references/content.schema.json`** — 위 스키마의 정식 JSON Schema(Draft 2020-12). 프로그램적 검증(Structured Outputs 등)에 사용.
- **`references/content.types.ts`** — 위 스키마의 TypeScript 타입 정의. 프론트엔드·백엔드 개발자용.
...
- **`references/industry-data.md`** — 업종별 기본 강점 후보 리스트, 이용흐름 템플릿, 예상 FAQ 질문 세트.
- **`references/input-questions.md`** — 정보가 부족할 때 사용자에게 물어보는 문항 순서(직접 입력, URL 크롤링 안 함).
