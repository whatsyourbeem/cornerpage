...
> ⚠️ **이 폴더(그리고 저장소에 편입된 뒤의 `spec/`)는 참고 문서가 아니라 빌드 의존성입니다.** `scripts/build-skill-prompt.ts`가 빌드 시점에 `skill/` 안의 파일들을 직접 읽어 `skill-prompt.generated.ts`를 만들고, `generate-content.ts`가 `schema/content.schema.json`을 런타임에 직접 import해서 ajv 검증에 씁니다. **이 폴더를 지우거나 옮기면 빌드가 깨집니다.** "그냥 문서니까 정리해도 되겠지"라고 판단하지 마세요.
...
│   ├── SKILL.md
│   ├── references/
│   │   ├── blocks.md, copywriting.md, schema.md,
│   │   │   prompt-schema-summary.md      (공통, 업종 무관)
│   │   ├── general/                       (범용 업종 대응, 기존 것)
│   │   │   ├── input-questions.md
│   │   │   └── industry-data.md
│   │   └── verticals/
│   │       └── fitness/                   (피트니스 전문화, 작성 중 — TODO 표시됨)
│   │           ├── input-questions.md
│   │           └── industry-data.md
│   └── evals/
│       └── evals.json
...
`skill/`을 실제 Claude API로 호출할 때(방법B, 상세는 별도 "백엔드 API 아키텍처" 문서), 시스템 프롬프트에 결합되는 파일은 정확히 이 5개입니다:
...
skill/references/{general 또는 verticals/해당업종}/industry-data.md
```
...
`SKILL.md` 1단계에 라우팅 규칙이 있습니다: 사업 정보가 피트니스/헬스장/PT스튜디오면 `verticals/fitness/`를, 그 외 모든 업종이면 `general/`을 씁니다. 실제 백엔드에서는 이 라우팅을 LLM이 실행 중에 고르는 게 아니라, **백엔드 코드가 요청 전에 미리 결정**해서 그에 맞는 시스템 프롬프트(둘 중 하나)를 골라 보내야 합니다(방법B 특성상).

`verticals/fitness/`는 현재 TODO 상태입니다 — 다음 단계에서 채웁니다.
...
생성된 콘텐츠 JSON이 스키마에 맞는지 확인하고 싶으면, 결과를 개발 세션(Claude와의 대화)에 붙여넣어 `schema/content.schema.json`으로 검증받으면 됩니다.
...
**경로 변경 필요**: `skill/`이 저장소 루트에서 `spec/skill/`로, `content.schema.json`이 `spec/schema/`로 이동하므로, 이걸 import/read하는 코드의 경로를 전부 갱신해야 합니다:
- `scripts/build-skill-prompt.ts`의 파일 읽기 경로(`skill/SKILL.md` 등 → `spec/skill/SKILL.md` 등)
- `src/lib/generate-content.ts`의 `import contentSchema from "..."` 경로 → `spec/schema/content.schema.json`
- `content.types.ts`를 프론트엔드/렌더러 쪽에서 별도로 복사해 쓰고 있었다면, 그것도 `spec/schema/content.types.ts`를 가리키도록 통일(중복 파일이 있었다면 이 기회에 제거)
...
- [ ] `verticals/fitness/input-questions.md` 실제 작성 (트레이너 자격증·PT 프로그램·회원 변화 사례 등 촘촘하게)
- [ ] `verticals/fitness/industry-data.md` 실제 작성 (강점 후보 세분화, FAQ 세트 확장)
- [ ] `skill/evals/fitness-evals.json` 신설 (피트니스 전용 테스트 케이스)
- [ ] 백엔드의 `build-skill-prompt.ts`가 업종별로 2개 프롬프트(general/fitness)를 만들도록 확장
