...
- **`schema-summary.md`** — 출력 JSON의 정확한 구조(구조 정의만, 완성 예시는 없음). **JSON 출력 직전 반드시 정독.** 완성 예시(콘텐츠 JSON 전체)가 보고 싶으면 `../for-frontend/fixtures/{vertical}/`의 실제 파일을 연다 — 예시를 이 파일이나 별도 문서에 복사해두지 않는다(과거 `schema.md`가 예시를 중복 보관하다 fixtures와 따로 노는 문제가 있어 폐기함, 2026-07-16).
