...
**사장님이 이미 쓰고 있는 시그니처 문구·슬로건을 줬다면(입력 폼에서 최대 40자로 미리 제한됨), 그 문구를 최우선으로 그대로 쓴다 — Claude가 새로 창작하지 않는다.** `copywriting.md` 3장("유저 원문 카피 존중")의 연장이다.
- 단어는 그대로 두고, 줄바꿈 규칙(위)을 지키기 위해 `\n`만 자연스러운 자리에 삽입한다(원문 훼손 최소화).
- 입력 폼에서 40자로 제한돼 있어 대부분 2줄(각 20자)에 무리 없이 들어간다. 그래도 자연스러운 끊을 자리가 없어 도저히 안 맞으면, `headline` 대신 `tagline`(길이 제약 없음)에 배치하는 것도 고려한다.
- 이 경우 "결과 약속 문구 금지" 원칙보다 원문 존중이 우선한다 — Claude가 지어낸 과장이 아니라 사장님이 실제로 걸고 있는 확립된 문구이기 때문이다(단, 원문이 근거 없는 최상급 표현이면 `copywriting.md` 4번 원칙에 따라 다듬어 제안).
- 시그니처 문구가 없으면(빈 값) 아래 원칙대로 Claude가 사실 기반으로 새로 작성한다.
