...
> ⚠️ **아직 general 복사본 그대로임 — boutique-fitness 전용 구조(신규 블록 3종: transformations·professionals·facility)로 분기 필요.** `blocks.md`가 완성되면 그 결론에 맞춰 갱신할 것.
>
...
>
> **이전엔 `schema.md`(완성 예시 포함, 사람이 읽는 문서)와 이 파일(구조 정의만, 프롬프트용)이 따로 있었으나, 완성 예시가 `../../for-frontend/fixtures/*.json`과 완전히 중복 저장되는 문제가 있어 `schema.md`를 삭제하고 이 파일로 통합했다(2026-07-16)** — 구조를 사람이 눈으로 확인할 때도 이 파일을 보면 되고, 완성 예시(콘텐츠 JSON 전체)가 필요하면 `../../for-frontend/fixtures/`를 직접 연다. 이제 이 파일이 이 vertical의 구조에 대한 **유일한 원본**이다 — 다른 곳에 복사해두지 않는다.
