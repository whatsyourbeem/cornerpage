...
1. **업종 라우팅 판단** — 사업 정보에서 업종을 확인한다. **피트니스/헬스장/PT스튜디오/짐**이면 `references/verticals/fitness/`의 `input-questions.md`·`industry-data.md`를 쓰고, 그 외 모든 업종이면 `references/general/`의 동일 파일명을 쓴다. 나머지 참고 파일(blocks.md·copywriting.md·schema.md 등)은 공통이라 업종과 무관하게 그대로 쓴다.
...
6. **JSON 출력** — `references/schema.md`의 스키마에 정확히 맞춰 출력한다. 필수 필드 누락 금지, 없는 데이터는 `null`.

각 단계의 상세 규칙은 아래와 참고 파일에 있다. **작업 전 반드시 `references/schema.md`(출력 형식)와, 판단이 필요한 시점에 해당 참고 파일을 읽는다.**
...
5. **강점 체크리스트가 빈약하면 카피도 빈약해진다.** 강점이 1~2개뿐이면 `references/general/industry-data.md`(또는 해당 업종의 `verticals/` 버전)의 업종별 기본 강점 후보를 참고해 각도를 넓힌다. 그래도 사실이 아닌 건 넣지 않는다.
...
미니홈페이지는 최대 11개 블록으로 구성된다. 각 블록은 3층으로 정의된다:
- **층 1 (슬롯 구성)** = JSON 필드 구조 (고정). → `references/schema.md`
- **층 2 (작성 원칙)** = 각 슬롯에 담길 내용의 "성질" 규정 (가드레일). → `references/blocks.md`
- **층 2.5 (좋은 예/나쁜 예)** = 품질 안정용 실제 사례. → `references/blocks.md`
...
- **`references/schema.md`** — 출력 JSON의 정확한 구조(사람이 읽는 문서). **JSON 출력 직전 반드시 정독.** 6개 업종의 완성 예시 포함.
- **`references/prompt-schema-summary.md`** — `schema.md`의 축약본(완성 예시 제외, 구조 정의만). **실제 프로덕션 시스템 프롬프트에는 이 파일이 `schema.md` 대신 포함된다**(토큰 비용 절감). 두 파일의 구조 설명(1~4장 상당 내용)은 항상 동기화되어야 한다 — 한쪽을 고치면 반드시 다른 쪽도 확인.
- **`references/blocks.md`** — 블록별 층2 작성 원칙 + 좋은/나쁜 예. 각 블록 콘텐츠 작성 시 참조.
- **`references/copywriting.md`** — 카피 생성 상세 규칙, AI 티 방지, 검증 가능한 사실 우선 원칙, "제안+근거" 방식.
- **`references/general/industry-data.md`** 또는 **`references/verticals/{업종}/industry-data.md`** — 업종별 기본 강점 후보 리스트, 이용흐름 템플릿, 예상 FAQ 질문 세트. 1번(업종 라우팅)에서 고른 쪽을 쓴다.
- **`references/general/input-questions.md`** 또는 **`references/verticals/{업종}/input-questions.md`** — 정보가 부족할 때 사용자에게 물어보는 문항 순서(직접 입력, URL 크롤링 안 함). 1번에서 고른 쪽을 쓴다.

**이 스킬 폴더 밖에 있는 것** (스킬 실행 시점에 참조하지 않음, 개발자용):
- **`../schema/content.schema.json`** — 정식 JSON Schema(Draft 2020-12). Claude에게 보내지 않음 — 백엔드가 API 응답을 받은 뒤 ajv로 검증할 때만 사용(방법B 채택 이유는 `백엔드 API 아키텍처` 문서 참고).
- **`../schema/content.types.ts`** — TypeScript 타입 정의. 프론트엔드·백엔드·렌더러 개발자용.
...
- [ ] 출력 직전 `references/schema.md`와 대조해 필드명·구조를 검증한다.
