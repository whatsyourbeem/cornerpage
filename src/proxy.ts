...
/**
 * {uuid}.cornerpage.co 서브도메인 라우팅.
 *
 * 실제 서비스에서는: cornerpage.co(루트)는 입력 폼, {uuid}.cornerpage.co는
 * 유저가 만든 미니홈페이지. 하나의 Next.js 배포가 와일드카드 DNS로 두 트래픽을
 * 전부 받고, 여기서 host 헤더를 보고 내부적으로 /site/[uuid]로 rewrite한다.
 * 그래서 신규 유저 생성 시 별도 빌드/배포 없이 DB row 하나로 즉시 서비스된다.
 *
